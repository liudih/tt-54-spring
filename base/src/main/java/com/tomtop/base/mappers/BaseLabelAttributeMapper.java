...
public interface BaseLabelAttributeMapper {
	...
	@Select({
			"select",
			"id, language_id, client_id,parent_id,label_id, is_change, level, category_id, attribute_key, attribute_value, sort ",
			"from base_label_attribute where is_enabled=1 and is_deleted=0 and category_id=#{0} order by sort" })
	@ResultMap("BaseResultMap")
	List<BaseLabelAttributeDto> getListByCategoryId(Integer categoryId);

	@Select({
			"select",
			"id, language_id, client_id,parent_id,label_id, is_change, level, category_id, attribute_key, attribute_value, sort ",
			"from base_label_attribute where is_enabled=1 and is_deleted=0 and attribute_key=#{0} order by sort" })
	@ResultMap("BaseResultMap")
	List<BaseLabelAttributeDto> getListByKey(String key);
