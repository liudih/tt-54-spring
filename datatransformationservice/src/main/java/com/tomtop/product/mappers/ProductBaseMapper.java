...
import com.tomtop.product.models.dto.base.NewArrival;
import com.tomtop.product.models.dto.base.ProductLite;
import com.tomtop.product.models.dto.base.ProductMessage;
import com.tomtop.product.models.dto.base.ProductNewarrivalsCalculateItem;
import com.tomtop.product.models.dto.base.SimpleProductBase;
import com.tomtop.product.models.dto.base.SkuRelateListingId;
...
public interface ProductBaseMapper {
	 ...
	 * @return
	 */
	@Select("select * from t_product_base order by iid "
			+ " limit #{0} offset #{1}")
	public List<ProductBase> getAllListingIds(int pageSize, int page);

	@Select("select * from t_product_base where dcreatedate >= #{2} and dcreatedate <= #{3}  order by iid "
			+ " limit #{0} offset #{1}")
	public List<ProductBase> getPageListByCreateDate(int pageSize, int page,
			Date start, Date end);
	...
	@Select("select tbase.clistingid,tbase.iwebsiteid,translate.ilanguageid,tbase.csku,istatus,dnewformdate,dnewtodate,bspecial"
			+ ",cvideoaddress,iqty,fprice,fcostprice,translate.ctitle,translate.cdescription as cdescription ,translate.cshortdescription,translate.ckeyword "
			+ ",translate.cmetatitle,translate.cmetakeyword,translate.cmetadescription "
			+ ",translate_default.ctitle ctitle_default,translate_default.cdescription cdescription_default,translate_default.cshortdescription cshortdescription "
			+ ",translate_default.ckeyword ckeyword_default,translate_default.cmetatitle cmetatitle_default,translate_default.cmetakeyword cmetakeyword_default "
			+ ",translate_default.cmetadescription cmetadescription_default "
			+ ",bmultiattribute,cparentsku,bvisible,bpulish FROM t_product_base tbase "
			+ " left join t_product_translate translate on tbase.clistingid=translate.clistingid "
			+ " left join t_product_translate translate_default on tbase.clistingid=translate_default.clistingid and translate_default.ilanguageid=translate.ilanguageid "
			+ " where tbase.clistingid =#{0} ")
	List<ProductBase> getProductBaseInfoByListingId(String listingID);
	...
	@Select("select * from t_product_base"
			+ "  ORDER BY iid  limit #{0} offset #{1}")
	public List<ProductBase> getProductByPage(int pageSize, int page);
	...
	List<ProductMessage> getProductMessagesByListingID(
			@Param("list") List<String> listingids, int languageid,
			Integer websiteId);
	...
	@Select("<script>"
			...
			+ "<foreach item='item' index='index' collection='list' open='(' separator=',' close=')' >#{item}</foreach>"
			+ "</script>")
	List<SkuRelateListingId> getSkuByListingIds(
			@Param("list") List<String> lintingIdList);
