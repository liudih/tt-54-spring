...
import com.tomtop.product.models.dto.Order;
import com.tomtop.product.models.dto.OrderDetail;
import com.tomtop.product.models.dto.OrderWithDtail;
...
public interface OrderMapper {
	...
	@Insert("INSERT INTO t_order (cemail, ccountrysn, ccountry, cstreetaddress, ccity, cprovince, cpostalcode, ctelephone, "
			...
			+ "cip, cshippingcode, cvhost, cordernumber) "
	...
	@Select("select * from t_order where cordernumber = #{0}")
	...
	public List<Order> getOrderByAidAndDate(@Param("sd") Date start,
			@Param("ed") Date end, @Param("list") List<Integer> statusIds);
	...
	@Select("select count(d.iqty) from t_order o "
			+ " inner join t_order_detail d on o.iid=d.iorderid where d.clistingid=#{0}")
	Integer getSaleCountByListingId(String listringId);

	@Select("select count(d.iqty) from t_order o "
			+ " inner join t_order_detail d on o.iid=d.iorderid where d.clistingid=#{0} and dcreatedate < #{1}")
	Integer getSaleCountByListingIdDate(String listringId, Date date);
