...
public class IndexServiceImpl implements IndexService {
	...
	public String insertBlukIndex(String indexName, String indexType,List<IndexEntity> prods) {
		...
		try{
			...
			Client client = BaseClient.getIndexClient(indexName, indexType);
			...
			if (bulkResponse.hasFailures()) {
				org.apache.commons.io.FileUtils.writeStringToFile(new File(Config.getValue("error.log.path")+"create_index_error.log"), bulkResponse.buildFailureMessage()+"\r\n",true);
	...
	public String insertIndex(String indexName, String indexType,IndexEntity product) {
		...
		try {
			Client client = BaseClient.getIndexClient(indexName, indexType);
			jsonProduct = new ObjectMapper().writeValueAsString(product);
			IndexResponse response = client.prepareIndex(indexName, indexType).setId(product.getListingId()).setSource(jsonProduct).get();
			if(StringUtils.isBlank(response.getIndex())){
				throw new Exception ("索引创建失败");
			}
		} catch (Exception ex) {
			String error_msg = indexName+"======"+indexType+"======"+jsonProduct;
			try {
				FileUtils.writeStringToFile(new File(Config.getValue("error.log.path")+"create_index_error.log"), error_msg+"\r\n",true);
			} catch (IOException e) {
				e.printStackTrace();
			}
			result = "500";
