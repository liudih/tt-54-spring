package com.tomtop.service.index.impl;
...
import com.tomtop.service.index.IBaseQueryBuild;;
/**
 * 构造查询条件
 * @author ztiny
 * @Date 2015-12-24
...
public abstract class BaseQueryBuildImpl implements IBaseQueryBuild{
	...
	public SearchRequestBuilder getRequestBuilder(PageBean bean) {
		...
		Client client =BaseClient.getSearchClient();
...
//		String fs[] = {"listingId","sku","brand","mutil.productTypes.productTypeName","mutil.keywords","mutil.items.value","spu"};
	...
	public QueryBuilder buildQueryBuilder(String key,List<Filter> list){
		...
		if(StringUtils.isNotBlank(key)){
			if((key.indexOf("-")!=-1|| key.indexOf(".")!=-1 || key.indexOf("#")!=-1 || key.indexOf("_")!=-1) && key.length()<30 && key.length()>4){
				queryBuilder = this.accurateMatch("sku", key);
			}else{
	 ...
	 * @return
	 */
	private SearchRequestBuilder autoOrder(SearchRequestBuilder requestBuilder,List<OrderEntity> orders,int productTypeId,String languageName){
	...
	public SearchRequestBuilder autoOrder(SearchRequestBuilder requestBuilder,PageBean bean,int productTypeId){
		...
		return autoOrder(requestBuilder,bean.getOrders(),productTypeId,bean.getLanguageName());
	}
	...
	public SearchRequestBuilder autoOrder(SearchRequestBuilder requestBuilder,PageBean bean){
		...
		return autoOrder(requestBuilder,bean.getOrders(),prodtypeId,bean.getLanguageName());
	}
